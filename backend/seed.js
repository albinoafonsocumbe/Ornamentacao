...
async function seed() {
    ...
    try {
        ...
        // 2. Eventos
        console.log('→ A inserir tipos de eventos...');
        await pool.query(`
        ...
        // 4. Portefólio
        console.log('→ A inserir portefólio...');
        await pool.query(`
        ...
        // 5. Tarifas para a Calculadora
        console.log('→ A inserir tarifas...');
        await pool.query(`
        ...
        // 6. Pedidos de exemplo para o Admin Dashboard
        console.log('→ A inserir pedidos de demonstração...');
        await pool.query(`
            INSERT INTO pedidos (nome_cliente, email, telefone, tipo_evento, data_evento, local_evento, descricao, estimativa, estado, notas_admin) VALUES
            ('Carla Machava', 'carla.machava@gmail.com', '+258 84 123 4567', 'Casamento', CURRENT_DATE + INTERVAL '45 days', 'Hotel Polana, Maputo', 'Casamento para 180 convidados em tons de branco e dourado champagne. Queremos túnel floral e pista espelhada.', 125000.00, 'aprovado', 'Reunião inicial agendada com os noivos para confirmação de paleta.'),
            ('Mário Cossa', 'mario.cossa@empresa.co.mz', '+258 82 987 6543', 'Corporativo', CURRENT_DATE + INTERVAL '20 days', 'Centro de Conferências Joaquim Chissano', 'Jantar de gala anual da empresa para 250 colaboradores. Exige iluminação com as cores da marca.', 85000.00, 'em_analise', 'A aguardar confirmação de plantas da sala.'),
            ('Anabela Sitoe', 'anabela.sitoe@outlook.com', '+258 87 555 1212', 'Batizado', CURRENT_DATE + INTERVAL '15 days', 'Jardim Privado, Matola', 'Batizado do pequeno Lucas com 60 convidados. Tons verde sage e detalhes dourados.', 32000.00, 'pendente', NULL),
            ('Eusébio Mondlane', 'eusebio.m@gmail.com', '+258 84 888 9999', 'Aniversário', CURRENT_DATE - INTERVAL '10 days', 'Restaurante Zambi, Maputo', 'Celebração de 50 anos com mesa de bolo espelhada e lounge VIP.', 55000.00, 'concluido', 'Evento executado com nota máxima do cliente.')
            ON CONFLICT DO NOTHING;
        `);

        console.log('✅ Base de dados povoada com sucesso com dados profissionais!');
    } catch (err) {
        console.error('⚠️ Erro ao povoar a base de dados:', err.message);
    } finally {
        await pool.end();
    }
}
