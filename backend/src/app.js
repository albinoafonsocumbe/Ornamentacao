...
const limitadorGeral = rateLimit({
    ...
    legacyHeaders: false,
    message: { sucesso: false, mensagem: 'Demasiadas tentativas. Tente mais tarde.' },
});
...
const limitadorLogin = rateLimit({
    windowMs: 15 * 60 * 1000,
    max: 10,
    message: { sucesso: false, mensagem: 'Demasiadas tentativas de login. Tente em 15 minutos.' },
});
...
const limitadorPedidos = rateLimit({
    ...
    max: 5,
    message: { sucesso: false, mensagem: 'Limite de pedidos atingido. Tente novamente mais tarde.' },
});
...
app.use((err, req, res, next) => {
    ...
    if (err.code === 'LIMIT_FILE_SIZE') {
        return res.status(413).json({ sucesso: false, mensagem: 'Ficheiro demasiado grande. Máximo 5MB.' });
    }
